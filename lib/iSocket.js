                                        ...
                                        pg_client.query ('SELECT * FROM hash WHERE hash ILIKE $1', [word.slice(1)], function (error, result) {
                                            if (!error) {
                                                ...
                                                if (result.rowCount == 0) {
                                                    pg_client.query ('INSERT INTO hash (hash, initiator, mention_ts[0]) VALUES ($1, $2, now()) RETURNING hash;', [word.slice(1), request.session.user.username], function (error, result) {
                                                        if (!error)
                                                            console.log ("#%s initiated", result.rows[0].hash);
                                                            // TODO: i think i should be emitting some stat on the hash
                                                    });
                                                }

                                                // posting under a hash
                                                else {
                                                    pg_client.query ('UPDATE hash SET mentions=$1, mention_ts[$2]=now() WHERE hash ILIKE $3 RETURNING hash, mentions;', [result.rows[0].mentions + 1, result.rows[0].mentions, result.rows[0].hash], function (error, result) {
                                                        if (!error)

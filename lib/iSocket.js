...
/// we'll be explicitly defining the memory session store
        ...
        configure: function (io, pg_client) {
            io.set ('authorization', function (handshakeData, callback) {
                ...
                if (handshakeData.headers.cookie) {
                    ...
                    // in the session the key is concatenated with with a '.' to other stuff, so i slice it --- 'it works fine' --- we all know what that means uh
            ...
            io.sockets.on ('connection', function (socket) {
                ...
                socket.on ("disconnect", function () {
                    ...
                    // destroying session
                    sessionStore.destroy (socket.handshake.cookie ['iMolly'].slice (2, socket.handshake.cookie ['iMolly'].indexOf ('.')), function () {
                        console.log (sessionStore);
                    });
                    */
                });
